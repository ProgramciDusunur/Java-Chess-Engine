...
import tahta.Tahta;
import test.Performans;

public class Grafik extends JPanel  {

    private Tahta t = new Tahta();
    private Performans p;
    ...
    public void paintComponent(Graphics g) {
        ...
        if (arayuzKararAl()) {
            ...
            //t.legalHamleler();            
            //t.arayuzCizilecekLegalHamleler(false,true,false);
            t.yeniArama();
            ...
            if (gerekli != null)
                for (int i = 0;i < gerekli.size();i++) {
                    if (gerekli.get(i) != "" && gerekli.get(i) != null && gerekli.get(i).charAt(1) >= 'a' && gerekli.get(i).charAt(1) <= 'h') {
                        g2D.setColor(new Color(255, 182, 193));
                        if (725-((Character.getNumericValue(gerekli.get(i).charAt(2)))*75)+75 >= 200) {
                            g2D.fill3DRect(kareBaslangicX+((Character.getNumericValue(gerekli.get(i).charAt(1))-9)*75)-75,725-((Character.getNumericValue(gerekli.get(i).charAt(2)))*75)+75, kareArtisX, kareArtisY, false);
                        }
