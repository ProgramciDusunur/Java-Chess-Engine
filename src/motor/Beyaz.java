...
public class Beyaz {	
		...
		for (int i = 0;i < tahta.length;i++) {
			...
			case 9:
				if (acmazTest(i,tahta[i],tahta,p) == 0) {
					if (p.getBeyazSahCekis() == 0) {vezir(i,i%8,i/8,tahta);} else if (p.getBeyazSahCekis() == 1) {sahCekisArama(tahta[i],i,tahta,p);}
				}				
				break;
			case 3:
				if (acmazTest(i,tahta[i],tahta,p) == 0) {
					if (p.getBeyazSahCekis() == 0) {at(i,i%8,i/8,tahta);} else if (p.getBeyazSahCekis() == 1) {sahCekisArama(tahta[i],i,tahta,p);}	
				}				
				break;
			case 4:
				if (acmazTest(i,tahta[i],tahta,p) == 0) {
					if (p.getBeyazSahCekis() == 0) {fil(i,i%8,i/8,tahta);} else if (p.getBeyazSahCekis() == 1) {sahCekisArama(tahta[i],i,tahta,p);}				
				}				
				break;
			case 5:
				if (acmazTest(i,tahta[i],tahta,p) == 0) {
					if (p.getBeyazSahCekis() == 0) {kale(i,i%8,i/8,tahta);} else if (p.getBeyazSahCekis() == 1) {sahCekisArama(tahta[i],i,tahta,p);}
				}				
				break;
			case 1:
				if (acmazTest(i,tahta[i],tahta,p) == 0) {
					if (p.getBeyazSahCekis() == 0) {piyon(i,i%8,i/8,tahta,p);} else if (p.getBeyazSahCekis() == 1) {sahCekisArama(tahta[i],i,tahta,p);}
				}				
				break;
			}
		}
	...
	public void piyon(int i, int x, int y, int[] tahta, PerformansTest p) {
		...
		if (x-1 > -1 && y == 1 && tahta[(y-1)*8+x-1] < 0) {hamle.add(9*100000000+(y-1)*10000+(x-1)*1000+(y)*100+(x)*10);hamle.add(4*10000000+(y-1)*10000+(x-1)*1000+(y)*100+(x)*10);hamle.add(3*10000000+(y-1)*10000+(x-1)*1000+(y)*100+(x)*10);hamle.add(5*10000000+(y-1)*10000+(x-1)*1000+(y)*100+(x)*10);}
	...
	public void sahCekisArama(int tas, int i, int[] tahta, PerformansTest k) {		 
		 ...
		 for (int p = 0;p < mudaheleKareleri.size();p++) {
			 ...
			 case 3:
				 ...
				 break;
			 case 1:
				 if (i/8 == 6 && tahta[(i/8-2)*8+i%8] == 0 && tahta[(i/8-1)*8+i%8] == 0 && (i/8-2)*8+i%8 == mudaheleKareleri.get(p)) {hamleEkle(10,i,i%8,i/8-2);}
	    			if ((i/8-1 > 0) && tahta[(i/8-1)*8+i%8] == 0 && ((i/8)-1)*8+i%8 == mudaheleKareleri.get(p)) {hamleEkle(tas,i,i%8,i/8-1);}
	    			if ((i/8-1 > 0 && i%8-1 > -1) && tahta[(i/8-1)*8+i%8-1] < 0 && (i/8-1)*8+i%8-1== mudaheleKareleri.get(p)) {hamleEkle(tas,i,i%8-1,i/8-1);}
	    			if ((i/8-1 > 0 && i%8+1 < 8) && tahta[(i/8-1)*8+i%8+1] < 0 && (i/8-1)*8+i%8+1== mudaheleKareleri.get(p)) {hamleEkle(tas,i,i%8+1,i/8-1);}
	...
	public void acmazHesap(int tasDeger, int hangiYon, int i, int y, int x, int sahTaraf, PerformansTest k) {		
		if (k.getBeyazSahCekis() == 0) {			
			switch(hangiYon) {			
			case 1:			
				acmazHesapCase1(tasDeger,i,y,x,sahTaraf);
				break;
			case 2:
				acmazHesapCase2(tasDeger,i,y,x,sahTaraf);
				break;
			case 3:
				acmazHesapCase3(tasDeger,i,y,x,sahTaraf);
				break;
			case 4:
				acmazHesapCase4(tasDeger,i,y,x,sahTaraf);
				break;
			case 5:
	...
	public void acmazHesapCase1(int tasDeger, int i, int y, int x, int sahTaraf) {
	...
	public void acmazHesapCase2(int tasDeger, int i, int y, int x, int sahTaraf) {
		for (int p = i/8+1,j = i%8;p <= y;p++) {				
			hamleEkle(tasDeger,i,j,p);				
		}
		for (int p = 1;p <= sahTaraf;p++) {
	...
	public void acmazHesapCase3(int tasDeger, int i, int y, int x, int sahTaraf) {
	...
	public void acmazHesapCase4(int tasDeger, int i, int y, int x, int sahTaraf) {
		for (int p = i/8,j = i%8+1;j <= x;j++) {
			hamleEkle(tasDeger,i,j,p);				
		}			
		for (int p = 1;p <= sahTaraf;p++) {
	...
	public int acmazKontrolCase8(int tasDeger,int hangiYon,int i,int sahTaraf,int[] tahta, PerformansTest k) {
	 ...
	 public void sahCekenTasNerede(int i, int j, PerformansTest p) {
		 int x = i%8, y = i/8, sahX, sahY;
	    	...
	    	if (p.getBeyazSahKonum()%10 == i%8 && p.getBeyazSahKonum()/10%10 > i/8) {
	    		sahX = p.getBeyazSahKonum()%10;sahY = p.getBeyazSahKonum()/10%10-1;
	    		for (;sahY >= y;sahY--) {
	    			sahMudaheleKare.add(sahY*8+sahX);
	    		}    		
	    	}
	    	else if (p.getBeyazSahKonum()%10 == i%8 && p.getBeyazSahKonum()/10%10 < i/8) {    		
	    		sahX = p.getBeyazSahKonum()%10;sahY = p.getBeyazSahKonum()/10%10+1;
	    		for (;sahY <= y;sahY++) {    			
	    			sahMudaheleKare.add(sahY*8+sahX);
	    		}
	    	}
	    	else if (p.getBeyazSahKonum()/10%10 == i/8 && p.getBeyazSahKonum()%10 > i%8) {
	    		sahX = p.getBeyazSahKonum()%10-1;sahY = p.getBeyazSahKonum()/10%10;	    		
	    		for (;sahX >= x;sahX--) {	    			
	    			sahMudaheleKare.add(sahY*8+sahX);
	    		}    		
	    	}
	    	...
	    	else if (((p.getBeyazSahKonum()/10%10)*8+(p.getBeyazSahKonum()%10)) %9 == i%9 && p.getBeyazSahKonum()/10%10 > i/8) {
	    		sahX = p.getBeyazSahKonum()%10-1;sahY = p.getBeyazSahKonum()/10%10-1;
	    	...
	    	else if (((p.getBeyazSahKonum()/10%10)*8+(p.getBeyazSahKonum()%10)) % 7 == i%7 && p.getBeyazSahKonum()/10%10 > i/8) {
	    		sahX = p.getBeyazSahKonum()%10+1;sahY = p.getBeyazSahKonum()/10%10-1;    		
	    		for (;sahX <= x && sahY >= y;sahX++,sahY--) {    			
	    			sahMudaheleKare.add(sahY*8+sahX);
	    		}
