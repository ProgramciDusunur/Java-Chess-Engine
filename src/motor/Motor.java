...
public class Motor extends Tahta {
	...
	int maksimumDerinlik = 2, toplamHamle = 0;
	public Motor() {
	...
	public int hareketTest(int derinlik, boolean siraKimde) {		
		...
		if (derinlik < maksimumDerinlik) {
			...
			pozisyonSayisi+= hamle.varyantlar.length()/6;			
			for (int i = 0;i < hamle.varyantlar.length();i+=6) {
				//System.out.println(hamle.varyantlar.substring(i, i+6));
				mevcutHamleNotasyon(Integer.parseInt(hamle.varyantlar.substring(i, i+6)),siraKimde);
				hamleYap(Integer.parseInt(hamle.varyantlar.substring(i, i+6)),false,siraKimde);
				//tahtayiGoster();
				//hamleYap(Integer.parseInt(hamle.varyantlar.substring(i, i+6)),true,siraKimde);
				hareketTest(derinlik+1,!siraKimde);
			}						
		}		
		return pozisyonSayisi;
	}	
	public void hamleYap(int i, boolean geriCevir,boolean siraKimde) {
		/*if (i > 0) {
			if (geriCevir) {
