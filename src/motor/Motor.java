...
public class Motor extends Oyun {
	...
	public Motor() {
		...
		//FEN("r3k2r/p1ppqpb1/bn2pnp1/3PN3/1p2P3/2N2Q1p/PPPBBPPP/R3K2R w KQkq -");
		FEN("2k2bnr/Rb4pp/1p3p2/1P1qp3/2n5/1QP3P1/3P1P1P/1NB1NRK1 b - - 0 18");
		performansTestKok(1,p.isSiraKimde(),p.getTahta());
		System.out.println("Toplam Hamle = "+toplamHamle+" Alınan taş sayısı = "+hamle.captures+" Çekilen şah sayısı "+hamle.toplamSahSayisi+" Geçerken Alma "+hamle.gecerkenAl+" Atılan rok sayısı "+hamle.rok);
	...
	public void performansTest(int derinlik, boolean siraKimde, double[] tahta) {
		if (derinlik < maksimumDerinlik) {
			...
			for (int i : hamleler) {
				...
				//tahtayiGoster(tahta);
				if (derinlik+1==maksimumDerinlik) {toplamHamle++;varyant++;}
				if (!hamle.matMi(!siraKimde, tahta, p)) {
					//System.out.println("Mat değil !");					
					performansTest(derinlik+1,!siraKimde,tahta);
				}
				t.hamleYap(i, true, siraKimde, tahta, p);
				//tahtayiGoster(tahta);
