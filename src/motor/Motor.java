...
public class Motor extends Oyun {
	...
	int alfaHamle = 0, betaHamle = 0;
	double alfa, beta;
	public Motor() {
		FEN("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1");
		long mili = System.nanoTime();	
		minimaxKok(1,p.isSiraKimde(),alfa,beta,true);
        ...
        System.out.println(mevcutHamleNotasyon(alfaHamle));
        System.out.println(mevcutHamleNotasyon(betaHamle));
	}
	public static void main(String [] args) {		
		new Motor();
	}
	public double minimaxKok(int derinlik, boolean siraKimde, double alfa, double beta, boolean maksimumYadaMinimumSkor) {
		int[] tahta = p.getTahta();		
		int[] hamleler = hamle.hamleleriAl(siraKimde, tahta, p);
		toplamHamle += hamleler.length;
		for (int i : hamleler) {
			t.hamleYap(i, false, siraKimde, tahta, p, derinlik-1);
			//tahtayiGoster(tahta);
			minimax(derinlik-1, !siraKimde, tahta,alfa,beta,!maksimumYadaMinimumSkor);
			t.hamleYap(i, true, siraKimde, tahta, p, derinlik-1);
		}
		return alfa;
	}
	public double minimax(int derinlik, boolean siraKimde, int[] tahta ,double alfa, double beta, boolean maksimumYadaMinimumSkor) {				
		if (derinlik == 0) {
			//tahtayiGoster(tahta);
			return h.analiz(tahta, siraKimde);
		}
		if (maksimumYadaMinimumSkor) {
			max(derinlik,siraKimde,tahta,alfa,beta);
		} else {
			min(derinlik,siraKimde,tahta,alfa,beta);
		}
		return 0.0;
	}
	public void max(int derinlik, boolean siraKimde, int[] tahta, double alfa, double beta) {		
		int[] hamleler = hamle.hamleleriAl(siraKimde, tahta, p);
		toplamHamle+= hamleler.length;
		for (int i : hamleler) {
			t.hamleYap(i, false, siraKimde, tahta, p, derinlik-1);
			...
			minimax(derinlik-1,!siraKimde,tahta,alfa,beta,false);
			//tahtayiGoster(tahta);
			t.hamleYap(i, true, siraKimde, tahta, p, derinlik-1);
		}
		//System.out.println(alfa);
		//System.out.println(mevcutHamleNotasyon((int)alfaHamle));
	}
	public void min(int derinlik, boolean siraKimde, int[] tahta, double alfa, double beta) {
		int[] hamleler = hamle.hamleleriAl(siraKimde, tahta, p);
		toplamHamle+= hamleler.length;
		for (int i : hamleler) {
			t.hamleYap(i, false, siraKimde, tahta, p, derinlik-1);
			...
			minimax(derinlik-1,!siraKimde,tahta,alfa,beta,true);
			t.hamleYap(i, true, siraKimde, tahta, p, derinlik-1);
		}
	}
