...
public class Tahta extends Oyun {
	...
	HashMap<Integer, Double> alinanTaslar = new HashMap<Integer, Double>();
	Hamleler hamle = new Hamleler();
	
	public void hamleYap(int i, boolean geriCevir, boolean siraKimde, double[] tahta, PerformansTest p, int pozisyonSira) {		
		switch(i/1000000) {
		case 2:
			if (geriCevir) {
		...
		case 900:
			if (geriCevir) {
				piyonTerfiGeriAl(i,siraKimde,tahta,p,pozisyonSira);
			} else {
	...
	public void piyonTerfi(int i, boolean siraKimde, double[] tahta, PerformansTest p, int pozisyonSira) {
		switch(i/1000000) {
		case 900:						
	...
	public void piyonTerfiGeriAl(int i, boolean siraKimde, double[] tahta, PerformansTest p, int pozisyonSira) {
	...
	public void piyonCiftGidis(int i, boolean siraKimde, double[] tahta, PerformansTest p) {
	...
	public void piyonCiftGidisGeriAl(int i, boolean siraKimde, double[] tahta, PerformansTest p) {
		p.setGecerkenAlma(-1);
		if (siraKimde) {
			tahta[((i/100)%10)*8+(i/10)%10] = 1.0;
		} else {
	...
	public void siradanGidisler(int i, boolean siraKimde, double[] tahta, PerformansTest p, int pozisyonSira) {		
		alinanTaslar.put(pozisyonSira, tahta[((i/10000)%10)*8+(i/1000)%10]);
		if (siraKimde) {
			if (i/100000 == 900) {												
	...
	public void siradanGidislerGeriAl(int i, boolean siraKimde, double[] tahta, PerformansTest p, int pozisyonSira) {
		tahta[((i/10000)%10)*8+(i/1000)%10] = alinanTaslar.get(pozisyonSira);
		alinanTaslar.remove(pozisyonSira);
		if (siraKimde) {
			if (i/100000 == 900) {												
